...
export function isSearchCacheValid(keyword) {
  ...
  if (cacheAgeMinutes > globals.searchCacheMinutes) {
    ...
    log(
      "info",
      `Search cache for "${keyword}" expired after ${cacheAgeMinutes.toFixed(
        2
      )} minutes`
    );
    return false;
...
export function setSearchCache(keyword, results) {
  ...
  log(
    "info",
    `Cached search results for "${keyword}" (${results.length} animes)`
  );
}
...
export function isCommentCacheValid(videoUrl) {
  ...
  if (cacheAgeMinutes > globals.commentCacheMinutes) {
    ...
    log(
      "info",
      `Comment cache for "${videoUrl}" expired after ${cacheAgeMinutes.toFixed(
        2
      )} minutes`
    );
    return false;
...
export function setCommentCache(videoUrl, comments) {
  ...
  log(
    "info",
    `Cached comments for "${videoUrl}" (${comments.length} comments)`
  );
}
...
export function addEpisode(url, title) {
  ...
  if (existingEpisode) {
    log(
      "info",
      `Episode with URL ${url} and title ${title} already exists in episodeIds, returning existing episode.`
    );
...
export function removeEpisodeByUrl(url) {
  ...
  globals.episodeIds = globals.episodeIds.filter(
  ...
  if (removedCount > 0) {
    log(
      "info",
      `Removed ${removedCount} episode(s) from episodeIds with URL: ${url}`
    );
    return true;
  }
...
export function addAnime(anime) {
  ...
  try {
    ...
    if (!anime.links || !Array.isArray(anime.links)) {
      log(
        "error",
        `Invalid or missing links in anime: ${JSON.stringify(anime)}`
      );
      return false;
    }
    ...
    anime.links.forEach((link) => {
      if (link.url) {
        ...
        log(
          "error",
          `Invalid link in anime, missing url: ${JSON.stringify(link)}`
        );
      }
    });
    ...
    const existingAnimeIndex = globals.animes.findIndex(
    ...
    log(
      "info",
      `animes: ${JSON.stringify(globals.animes, (key, value) =>
        key === "links" ? value.length : value
      )}`
    );
...
export function cleanupExpiredIPs(currentTime) {
  ...
  if (cleanedCount > 0) {
    log(
      "info",
      `[Rate Limit] Cleanup completed: removed ${cleanedCount} expired IP records`
    );
  }
}

// 获取豆瓣解析缓存
export function getDoubanCache(key) {
  if (globals.doubanCache.has(key)) {
    log("info", `Using douban cache for "${key}"`);
    return globals.doubanCache.get(key);
  }
  return null;
}

// 设置豆瓣解析缓存
export function setDoubanCache(key, url) {
  globals.doubanCache.set(key, url);
  log("info", `Cached douban url for "${key}": ${url}`);
}
...
export async function getLocalCaches() {
  if (!globals.localCacheInitialized) {
    try {
      ...
      globals.animes =
        JSON.parse(readCacheFromFile("animes")) || globals.animes;
      globals.episodeIds =
        JSON.parse(readCacheFromFile("episodeIds")) || globals.episodeIds;
      globals.episodeNum =
        JSON.parse(readCacheFromFile("episodeNum")) || globals.episodeNum;
      ...
      if (lastSelectMapData) {
        globals.lastSelectMap = new Map(
          Object.entries(JSON.parse(lastSelectMapData))
        );
        log(
          "info",
          `Restored lastSelectMap from local cache with ${globals.lastSelectMap.size} entries`
        );
      }

      // 恢复 doubanCache 并转换为 Map 对象
      const doubanCacheData = readCacheFromFile("doubanCache");
      if (doubanCacheData) {
        globals.doubanCache = new Map(
          Object.entries(JSON.parse(doubanCacheData))
        );
        log(
          "info",
          `Restored doubanCache from local cache with ${globals.doubanCache.size} entries`
        );
      }

      // 更新哈希值
      globals.lastHashes.animes = simpleHash(JSON.stringify(globals.animes));
      globals.lastHashes.episodeIds = simpleHash(
        JSON.stringify(globals.episodeIds)
      );
      globals.lastHashes.episodeNum = simpleHash(
        JSON.stringify(globals.episodeNum)
      );
      globals.lastHashes.episodeNum = simpleHash(
        JSON.stringify(globals.episodeNum)
      );
      globals.lastHashes.lastSelectMap = simpleHash(
        JSON.stringify(Object.fromEntries(globals.lastSelectMap))
      );
      globals.lastHashes.doubanCache = simpleHash(
        JSON.stringify(Object.fromEntries(globals.doubanCache))
      );
...
export async function updateLocalCaches() {
  try {
    ...
    const variables = [
      ...
      { key: "doubanCache", value: globals.doubanCache },
    ];

    for (const { key, value } of variables) {
      // 对于 lastSelectMap 和 doubanCache（Map 对象），需要转换为普通对象后再序列化
      const serializedValue =
        key === "lastSelectMap" || key === "doubanCache"
    ...
    if (updates.length > 0) {
      log(
        "info",
        `Updated local caches for keys: ${updates.map((u) => u.key).join(", ")}`
      );
    ...
    log(
      "error",
      `Error details - Name: ${error.name}, Cause: ${
        error.cause ? error.cause.message : "N/A"
      }`
    );
  }
}
