...
export function convertToDanmakuJson(contents, platform) {
  ...
  if (typeof contents === "string") {
    // 处理 XML 字符串
    items = [...contents.matchAll(/<d p="([^"]+)">([^<]+)<\/d>/g)].map(
      (match) => ({
        p: match[1],
        m: match[2],
      })
  ...
  // 切割字符串成正则表达式数组
  const regexArray = globals.blockedWords
    .split(/(?<=\/),(?=\/)/)
    .map((str) => {
      // 去除两端的斜杠并转换为正则对象
  ...
  const regexArrayToString = (array) =>
    Array.isArray(array)
      ? array.map((regex) => regex.toString()).join("\n")
      : String(array);
  ...
  if (
    ...
    convertedDanmus = groupedDanmus.map((danmu) => {
      ...
      let colors = [
        ...
        16777215, 16744319, 16752762, 16774799, 9498256, 8388564, 8900346,
        14204888, 16758465,
      ];
...
export function convertDanmuToXml(danmuData) {
  ...
  if (Array.isArray(comments)) {
    for (const comment of comments) {
      ...
      xml +=
        '    <d p="' +
        escapeXmlAttr(pValue) +
        '">' +
        escapeXmlText(comment.m) +
        "</d>\n";
    }
  }
...
export function postProcessDanmu(danmuData) {
  if (!danmuData || !Array.isArray(danmuData.comments)) {
    return { danmu: 0, danmuku: [] };
  }
  ...
  const danmuku = danmuData.comments.map((item) => {
    const pValues = item.p.split(",");
    // 获取时间
    const time = item.t !== undefined ? item.t : parseFloat(pValues[0]);

    // 获取类型
    const typeKey = pValues[1] || "1";
    const type = typeMap[typeKey] || "right";

    // 获取颜色并转换为 Hex
    let colorInt = parseInt(pValues[2], 10);
    if (isNaN(colorInt)) {
      // 尝试从第4位获取（旧格式可能是4字段）或者默认白色
      colorInt = parseInt(pValues[3], 10);
      if (isNaN(colorInt)) colorInt = 16777215;
    }
