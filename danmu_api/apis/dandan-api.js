...
import {
  ...
  removeEarliestAnime,
  resolveAnimeById,
  resolveAnimeByIdFromDetailStore,
  setPreferByAnimeId,
  setSearchCache,
  storeAnimeIdsToMap,
  writeCacheToFile,
...
import {
  formatDanmuResponse,
  convertToDanmakuJson,
} from "../utils/danmu-util.js";
...
import {
  ...
  mergeDanmakuList,
  MERGE_DELIMITER,
  alignSourceTimelines,
} from "../utils/merge-util.js";
...
function shouldIncludeVideoDuration(queryFormat, includeDuration = false) {
  ...
  const format = String(
    queryFormat || globals.danmuOutputFormat || "json",
  ).toLowerCase();
  return format === "json";
}
...
function extractDurationFromSegments(segmentResult) {
  const explicitDuration = normalizeDurationValue(
    segmentResult?.duration || segmentResult?.videoDuration || 0,
  );
  if (explicitDuration > 0) return explicitDuration;

  const segmentList = Array.isArray(segmentResult?.segmentList)
    ? segmentResult.segmentList
    : [];
...
async function resolveMergedDuration(url) {
  ...
  try {
    const targetUrls = url.includes(MERGE_DELIMITER)
      ? extractMergedUrls(url)
      : [url];
    const durations = await Promise.all(targetUrls.map(resolveUrlDuration));
    return durations.reduce(
      (maxValue, currentValue) => Math.max(maxValue, currentValue || 0),
      0,
    );
  } catch (error) {
    log("warn", `[Duration] 获取时长失败: ${error.message}`);
...
export function matchSeason(anime, queryTitle, season) {
  ...
  if (normalizedAnimeTitle.includes(normalizedQueryTitle)) {
    ...
    if (title.startsWith(normalizedQueryTitle)) {
      ...
      const chineseNumber = afterTitle.match(
...
export async function searchAnime(
  ...
  const requestAnimeDetailsMap =
    detailStore instanceof Map ? detailStore : new Map();
  ...
  if (urlRegex.test(queryTitle)) {
    ...
    addAnime(
      Anime.fromJson({...tmpAnime, links: links}),
  ...
  try {
    ...
    const requestPromises = globals.sourceOrderArr.map((source) => {
      ...
      if (source === "vod")
        return vodSource.search(queryTitle, preferAnimeId, preferSource);
    ...
    for (const key of globals.sourceOrderArr) {
      if (key === "360") {
        // 等待处理360来源
        await kan360Source.handleAnimes(
          animes360,
          queryTitle,
          curAnimes,
          requestAnimeDetailsMap,
        ...
        if (animesVodResults && Array.isArray(animesVodResults)) {
          for (const vodResult of animesVodResults) {
            if (vodResult && vodResult.list && vodResult.list.length > 0) {
              await vodSource.handleAnimes(
                vodResult.list,
                queryTitle,
                curAnimes,
                vodResult.serverName,
                requestAnimeDetailsMap,
              );
            }
          }
        ...
        // 等待处理Douban来源
        await doubanSource.handleAnimes(
          animesDouban,
          queryTitle,
          curAnimes,
          requestAnimeDetailsMap,
        ...
        // 等待处理Renren来源
        await renrenSource.handleAnimes(
          animesRenren,
          queryTitle,
          curAnimes,
          requestAnimeDetailsMap,
        ...
        // 等待处理Hanjutv来源
        await hanjutvSource.handleAnimes(
          animesHanjutv,
          queryTitle,
          curAnimes,
          requestAnimeDetailsMap,
        ...
        // 等待处理Bahamut来源
        await bahamutSource.handleAnimes(
          animesBahamut,
          queryTitle,
          curAnimes,
          requestAnimeDetailsMap,
        ...
        // 等待处理弹弹play来源
        await dandanSource.handleAnimes(
          animesDandan,
          queryTitle,
          curAnimes,
          requestAnimeDetailsMap,
        ...
        // 等待处理自定义弹幕源来源
        await customSource.handleAnimes(
          animesCustom,
          queryTitle,
          curAnimes,
          requestAnimeDetailsMap,
        ...
        // 等待处理Tencent来源
        await tencentSource.handleAnimes(
          animesTencent,
          queryTitle,
          curAnimes,
          requestAnimeDetailsMap,
        ...
        // 等待处理Youku来源
        await youkuSource.handleAnimes(
          animesYouku,
          queryTitle,
          curAnimes,
          requestAnimeDetailsMap,
        ...
        // 等待处理iQiyi来源
        await iqiyiSource.handleAnimes(
          animesIqiyi,
          queryTitle,
          curAnimes,
          requestAnimeDetailsMap,
        ...
        // 等待处理Mango来源
        await mangoSource.handleAnimes(
          animesImgo,
          queryTitle,
          curAnimes,
          requestAnimeDetailsMap,
        ...
        // 等待处理Bilibili来源
        await bilibiliSource.handleAnimes(
          animesBilibili,
          queryTitle,
          curAnimes,
          requestAnimeDetailsMap,
        ...
        // 等待处理Leshi来源
        await leshiSource.handleAnimes(
          animesLeshi,
          queryTitle,
          curAnimes,
          requestAnimeDetailsMap,
        ...
        // 等待处理Xigua来源
        await xiguaSource.handleAnimes(
          animesXigua,
          queryTitle,
          curAnimes,
          requestAnimeDetailsMap,
        ...
        // 等待处理Maiduidui来源
        await maiduiduiSource.handleAnimes(
          animesMaiduidui,
          queryTitle,
          curAnimes,
          requestAnimeDetailsMap,
        ...
        // 等待处理Animeko来源
        await animekoSource.handleAnimes(
          animesAnimeko,
          queryTitle,
          curAnimes,
          requestAnimeDetailsMap,
  ...
  if (globals.enableAnimeEpisodeFilter) {
    ...
    for (const anime of curAnimes) {
      ...
      if (
        globals.animeTitleFilter &&
        globals.animeTitleFilter.test(animeTitle)
      ) {
        log(
          "info",
      ...
      if (animeData && animeData.links) {
        ...
        log(
          "info",
          `[searchAnime] Anime ${anime.animeId} filtered episodes: ${episodesList.length}/${animeData.links.length}`,
        );
...
function filterSameEpisodeTitle(filteredTmpEpisodes) {
  const filteredEpisodes = filteredTmpEpisodes.filter(
...
function getPlatformMatchScore(candidatePlatform, targetPlatform) {
  ...
  // 预处理：按 & 分割，转小写，去空格
  const cParts = candidatePlatform
    .split("&")
    .map((s) => s.trim().toLowerCase())
    .filter((s) => s);
  const tParts = targetPlatform
    .split("&")
    .map((s) => s.trim().toLowerCase())
    .filter((s) => s);
...
async function matchAniAndEpByAi(
  season,
  episode,
  year,
  searchData,
  ...
  if (!globals.aiValid || !aiMatchPrompt) {
    log(
      "warn",
      "AI configuration is incomplete, falling back to normal matching",
    );
  ...
  try {
    ...
    if (season && episode) {
      // 剧集模式逻辑
      const filteredTmpEpisodes = bangumiData.bangumi.episodes.filter(
        (episode) => {
          return !globals.episodeTitleFilter.test(episode.episodeTitle);
        },
      );
      ...
      log(
        "info",
        "过滤后的集标题",
        filteredEpisodes.map((episode) => episode.episodeTitle),
      );
...
function getBangumiDataForMatch(anime, detailStore = null) {
  ...
  if (!detailAnime) {
    log(
      "warn",
      `[matchAnime] Missing request detail snapshot for anime ${anime?.animeId ?? anime?.bangumiId}`,
    );
    return null;
  }

  return buildBangumiData(
    detailAnime,
    anime?.bangumiId || anime?.animeId || "",
  );
}

function computeTargetEpisode(
  offsets,
  season,
  episode,
  filteredEpisodes,
  ...
  const offsetIndex = filteredEpisodes.findIndex(
    (episode) => episode.episodeTitle === offsetEpisodeTitle,
  );
  if (offsetIndex !== -1) {
    // 计算本次获取的目标index
    ...
    log(
      "info",
      `Applying offset "${offsets[seasonKey]}" for S${season}E${episode} -> ${targetEpisode}`,
    );
  }
  return targetEpisode;
}

async function matchAniAndEp(
  season,
  episode,
  year,
  searchData,
  ...
  for (const anime of searchData.animes) {
    ...
    if (season && episode) {
      ...
      if (normalizeSpaces(anime.animeTitle).includes(normalizedTitle)) {
        ...
        if (!matchYear(anime, year)) {
          log(
            "info",
            `Year mismatch: anime year ${extractYear(anime.animeTitle)} vs query year ${year}`,
          );
          continue;
        }
    ...
    if (season && episode) {
      ...
      if (offsets && offsets[String(season)] !== undefined) {
        targetEpisode = computeTargetEpisode(
          offsets,
          season,
          episode,
          filteredEpisodes,
      ...
      // 匹配集数 (注意：findEpisodeByNumber 已增强支持模糊平台匹配)
      matchedEpisode = findEpisodeByNumber(
        filteredEpisodes,
        targetEpisode,
        platform,
      );
    ...
    if (matchedEpisode) {
      ...
      const actualPlatform =
        extractPlatformFromTitle(anime.animeTitle) || anime.source;
      let currentScore = 0;
...
async function fallbackMatchAniAndEp(
  ...
  for (const anime of searchData.animes) {
    ...
    if (year && !matchYear(anime, year)) {
      log(
        "info",
        `Fallback: Year mismatch: anime year ${extractYear(anime.animeTitle)} vs query year ${year}`,
      );
      continue;
    }
    ...
    if (season && episode) {
      ...
      // 使用新的集数匹配策略
      const matchedEpisode = findEpisodeByNumber(
        filteredEpisodes,
        targetEpisode,
        null,
      );
...
export async function extractTitleSeasonEpisode(cleanFileName) {
  ...
  if (match) {
    ...
    const yearMatch = cleanFileName.match(
      /(?:\.|\(|（)((?:19|20)\d{2})(?:\)|）|\.|$)/,
    );
    if (yearMatch) {
    ...
    // 情况3：中文+英文混排（如 爱情公寓.ipartment.2009）
      ...
      const beforeYear = title.split(
        /\.(?:19|20)\d{2}|2160p|1080p|720p|H265|iPhone/,
      )[0];
    ...
    const titleRegex =
      /^([^.\s]+(?:[.\s][^.\s]+)*?)(?:[.\s](?:\d{4}|(?:19|20)\d{2}|\d{3,4}p|S\d+|E\d+|WEB|BluRay|Blu-ray|HDTV|DVDRip|BDRip|x264|x265|H\.?264|H\.?265|AAC|AC3|DDP|TrueHD|DTS|10bit|HDR|60FPS))/i;
  ...
  log("info", "Parsed title, season, episode, year", {
    title,
    season,
    episode,
    year,
  ...
  try {
      ...
      return jsonResponse(
        {
          errorCode: 400,
          success: false,
          errorMessage: "Missing fileName parameter",
        },
        400,
      );
    }
    ...
    if (globals.titleMappingTable && globals.titleMappingTable.size > 0) {
      ...
      if (mappedTitle) {
        ...
        log(
          "info",
          `Title mapped from original: ${url.searchParams.get("keyword")} to: ${title}`,
        );
      }
    }
    ...
    const [preferAnimeId, preferSource, offsets] = getPreferAnimeId(
      title,
      season,
    );
    ...
    let originSearchUrl = new URL(
      req.url.replace("/match", `/search/anime?keyword=${title}`),
    );
    ...
    if (aiMatchResult.resAnime) {
      ...
      log(
        "info",
        `AI match found: ${resAnime.animeTitle}; episode: ${resEpisode.episodeTitle}`,
      );
    } else {
      // AI匹配失败或未配置，使用传统匹配方式
      for (const platform of dynamicPlatformOrder) {
        const __ret = await matchAniAndEp(
          season,
          episode,
          year,
          searchData,
      ...
      if (!resAnime) {
        const __ret = await fallbackMatchAniAndEp(
          searchData,
          req,
          season,
          episode,
  ...
  for (const animeItem of searchData.animes) {
    ...
    if (
      ...
      if (episode) {
        if (episode === "movie") {
          // 仅保留剧场版结果
          filteredEpisodes = bangumiData.bangumi.episodes.filter(
            (ep) =>
          ...
          filteredEpisodes = bangumiData.bangumi.episodes.filter(
            (ep) => parseInt(ep.episodeNumber) === targetEpisode,
          );
        }
      }
...
function buildBangumiData(anime, idParam = "") {
  ...
  if (globals.enableAnimeEpisodeFilter) {
    ...
    log(
      "info",
      `[getBangumi] Episode filter enabled. Filtered episodes: ${episodesList.length}/${anime.links.length}`,
    );
    ...
    if (episodesList.length === 0) {
      log(
        "warn",
        `[getBangumi] No valid episodes after filtering for anime ID ${idParam || anime.bangumiId}`,
      );
      return {
        errorCode: 404,
        success: false,
...
async function fetchMergedComments(url, animeTitle, commentId) {
  ...
  if (sourceNames.includes("dandan")) {
    ...
    const dandanShifts =
      results[dandanIndex] && results[dandanIndex].relatedShifts
        ? results[dandanIndex].relatedShifts
        : {};
  ...
  if (globals.danmuOffsetRules?.length > 0 && animeTitle && commentId) {
    ...
    if (episodeTitle) {
      let {baseTitle, season, episode} = extractAnimeInfo(
        animeTitle,
        episodeTitle,
      );
      season ||= 1;
      ...
      results.forEach((list, idx) => {
        ...
        if (offset !== 0 && Array.isArray(list)) {
          log(
            "info",
            `[Merge] 应用偏移 ${offset}s -> ${sourceNames[idx]} (${baseTitle}/${seasonStr}/${episodeStr})`,
          );
  ...
  const statDetails = Object.entries(stats)
    .map(([k, v]) => `${k}: ${v}`)
    .join(", ");
  log(
    "info",
    `[Merge] 聚合原始数据完成: 总计 ${mergedList.length} 条 (${statDetails})`,
  );
...
export async function getComment(
  ...
  let plat = title
    ? (title.match(/【(.*?)】/) || [null])[0]?.replace(/[【】]/g, "")
    : null;
  const shouldAttachDuration = shouldIncludeVideoDuration(
    queryFormat,
    includeDuration,
  );
  ...
  let danmus = [];
  const durationPromise = shouldAttachDuration
    ? resolveMergedDuration(url)
    : null;
  ...
  if (animeId && source) {
    ...
    if (clientIp) {
      ...
      if (
        ...
        log(
          "info",
          `Calculated episode offset for IP ${clientIp}: Query E${lastSearch.episode}, Selected ${episodeTitle} -> Offset ${offset} (Season ${lastSeason})`,
        );
      }
    }
    ...
    log(
      "info",
      `animeTitle：${animeTitle}; lastTitle：${lastTitle}; titleMatches：${titleMatches(animeTitle, lastTitle)}`,
    );
  ...
  if (
    animeTitle &&
    episodeTitle &&
    globals.danmuOffsetRules?.length > 0 &&
    !(url && url.includes(MERGE_DELIMITER))
  ) {
    let {baseTitle, season, episode} = extractAnimeInfo(
      animeTitle,
      episodeTitle,
    ...
    if (offset !== 0) {
      log(
        "info",
        `Applying danmu offset: ${offset}s for ${baseTitle}/${seasonStr}/${episodeStr}`,
      );
...
export async function getCommentByUrl(
  videoUrl,
  queryFormat,
  segmentFlag,
  includeDuration = false,
  ...
  try {
    ...
    if (url.includes(".qq.com")) {
      ...
      const urlPattern = /^(https?:\/\/)?([\w.-]+)\.([a-z]{2,})(\/.*)?$/i;
      if (urlPattern.test(url)) {
        danmus = await otherSource.getComments(
          url,
          "other_server",
          segmentFlag,
        );
...
export async function getSegmentComment(segment, queryFormat) {
  try {
    ...
    log(
      "info",
      `Successfully fetched ${danmus.length} segment comments from URL`,
    );
