...
async function handleRequest(req, env, deployPlatform, clientIp) {
  ...
  if (globals.ipBlacklist?.length) {
    const isBlocked = globals.ipBlacklist.some((rule) =>
      matchIpBlacklistRule(rule, clientIp),
    );
    if (isBlocked) {
  ...
  const isValidToken =
    firstPart === globals.token || firstPart === globals.adminToken;
  ...
  if (targetPaths.some((targetPath) => path.includes(targetPath))) {
    ...
    const lastRecord =
      globals.reqRecords.length > 0
        ? globals.reqRecords[globals.reqRecords.length - 1]
        : null;
  ...
  if (globals.token === "87654321") {
    ...
    if (
      parts.length < 1 ||
      (parts[0] !== globals.token && parts[0] !== globals.adminToken)
    ) {
  ...
  if (
    ...
    if (
      ...
      if (path.startsWith("/v2/") || path === "/v2") {
        ...
        log(
          "info",
          `[Path Check] Path is missing /api/v2 prefix. Adding /api/v2...`,
        );
  ...
  if (path.startsWith("/api/v2/comment") && method === "GET") {
    ...
    if (videoUrl) {
      ...
      if (cachedComments !== null) {
        log(
          "info",
          `[Rate Limit] Cache hit for URL: ${videoUrl}, skipping rate limit check`,
        );
        return getCommentByUrl(
          videoUrl,
          queryFormat,
          segmentFlag,
          includeDuration,
      ...
      if (globals.rateLimitMaxRequests > 0) {
        ...
        const recentRequests = history.filter(
          (timestamp) => currentTime - timestamp <= oneMinute,
        );
        ...
        if (recentRequests.length >= globals.rateLimitMaxRequests) {
          log(
            "warn",
            `[Rate Limit] IP ${clientIp} exceeded rate limit (${recentRequests.length}/${globals.rateLimitMaxRequests} requests in 1 minute)`,
          );
          return jsonResponse(
            {
              errorCode: 429,
        ...
        log(
          "info",
          `[Rate Limit] IP ${clientIp} request count: ${recentRequests.length}/${globals.rateLimitMaxRequests}`,
        );
      }
      ...
      return jsonResponse(
        {
          errorCode: 400,
          success: false,
          errorMessage: "Missing commentId or url parameter",
        },
        400,
      );
    }
    ...
    if (urlForComment) {
      ...
      if (cachedComments !== null) {
        log(
          "info",
          `[Rate Limit] Cache hit for URL: ${urlForComment}, skipping rate limit check`,
        );
        return getComment(
          path,
          queryFormat,
...
function isIpInCidr(ip, cidrIp, prefix) {
  ...
  if (ipInt === null || cidrInt === null || prefix < 0 || prefix > 32)
    return false;
...
export default {
  async fetch(request, env, ctx) {
    ...
    return handleRequest(
      request,
      env,
      isRunningOnVercel() ? "vercel" : "cloudflare",
      clientIp,
    );
  },
};
